...
int main(int argc, char* argv[]) {
    ...
    */
    ...
    resSerial.open("export/O1/serial.csv", std::ios::app);
    if (!resSerial.is_open()) {
        std::cerr << "Could not open file export/O1/serial.csv" << std::endl;
        return -1;
    }
    resSerial << "iteration, time, size" << std::endl;

    resImpParallelism.open("export/O1/implicit_parallelism.csv", std::ios::app);
    if (!resImpParallelism.is_open()) {
        std::cerr << "Could not open file export/O1/implicit_parallelism.csv" << std::endl;
        return -1;
    }
    resImpParallelism << "iteration, time, size" << std::endl;

    resOpenMP.open("export/O1/openMP.csv", std::ios::app);
    if (!resOpenMP.is_open()) {
        std::cerr << "Could not open file export/O1/openMP.csv" << std::endl;
        return -1;
    }

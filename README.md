...
The final executable will be located at `` bin/run/vectorization.out ``.
...
```shell
./bin/run/vectorization.out -s 12 # sizes: 2^12
```

Set a range of matrix sizes:
```shell
./bin/run/vectorization.out -m 4 -M 12 # sizes: 2^4 -> 2^12
```

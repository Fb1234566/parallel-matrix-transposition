...
CPPFLAGSIM = -Iinclude -msse3 -mfpmath=sse -O1 -std=c++11 # implicit parallelism cserial_cpp_filesompilation flags
...
run: $(obj_files)
	...
	mkdir export/O1 -p && \
	mkdir bin/run -p && \
	g++ $(obj_files) bin/obj/main.o -o bin/run/O1.out -std=c++11 -fopenmp

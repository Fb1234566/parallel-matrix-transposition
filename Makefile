...
CPPFLAGSIM = -Iinclude -ftree-vectorize -msse3 -mfpmath=sse -std=c++11 # implicit parallelism cserial_cpp_filesompilation flags
...
run: $(obj_files)
	...
	mkdir export/vectorization -p && \
	mkdir bin/run -p && \
	g++ $(obj_files) bin/obj/main.o -o bin/run/vectorization.out -std=c++11 -fopenmp
